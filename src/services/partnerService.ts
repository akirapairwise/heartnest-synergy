...
export const createInvitation = async (userId: string, recipientEmail: string) => {
  try {
    ...
    // To solve the TypeScript error, we need to tell TypeScript to trust us that
    // the invitation_code will be generated by a database trigger
    const { data, error } = await supabase
      .from('partner_invitations')
      .insert({
        sender_id: userId,
        ...
        status: 'pending',
        // Adding a dummy value to satisfy TypeScript, this will be overwritten by the database trigger
        invitation_code: 'PENDING-WILL-BE-REPLACED-BY-TRIGGER'
      })
      .select()
      .single();
