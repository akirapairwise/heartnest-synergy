...
serve(async (req) => {
  ...
  try {
    ...
    const { initiatorStatement, responderStatement, conflictId } = await req.json();

    if (!initiatorStatement || !responderStatement || !conflictId) {
      throw new Error('Missing required parameters');
    }

    console.log(`Processing conflict resolution for conflict ID: ${conflictId}`);
...
`;
    ...
    console.error('Error in generate-conflict-resolution function:', error);
